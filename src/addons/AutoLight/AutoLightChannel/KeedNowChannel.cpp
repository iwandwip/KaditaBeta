...
#include "KeedNowChannel.h"

#define setHigh(...) setStateHigh(__VA_ARGS__, -1)
#define setLow(...) setStateLow(__VA_ARGS__, -1)

KeedNowChannel::KeedNowChannel()
        : sequence(0), ioTimer(40), taskTemp(nullptr),
          sequences{&KeedNowChannel::off,
                    &KeedNowChannel::taskSequence0,
                    &KeedNowChannel::taskSequence1,
                    &KeedNowChannel::taskSequence2,
                    &KeedNowChannel::taskSequence3,
                    &KeedNowChannel::taskSequence4,
                    &KeedNowChannel::taskSequence5,
                    &KeedNowChannel::taskSequence6,
                    &KeedNowChannel::on} {}

void KeedNowChannel::init(IOExpander **_ioBase, configuration_t _cfg) {
...
void KeedNowChannel::run() {
    update();
}

void KeedNowChannel::setInterruptConfig(interrupt_t _cfg) {
    isr = _cfg;
}

void KeedNowChannel::changeModes() {
...
void KeedNowChannel::setBaseDelay(uint32_t _time) {
    ioTimer = _time;
}

void (KeedNowChannel::*KeedNowChannel::getSequence(uint8_t index))() {
    return sequences[index];
}

void KeedNowChannel::taskSequence0() {
    // blink ////////////////////////////////////////
    {
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 15; ++j) {
...
void KeedNowChannel::taskSequence1() {
    // fill 2 point ////////////////////////////////////////
    {
...
void KeedNowChannel::taskSequence2() {
    // fill right ////////////////////////////////////////
    {
        for (int i = cfg.pin_size; i > 0; --i) {
            for (int j = 0; j < i; j++) {
...
void KeedNowChannel::taskSequence3() {
    // fill in ////////////////////////////////////////
    {
        for (int j = 0; j < cfg.pin_size / 2; ++j) {
...
void KeedNowChannel::taskSequence4() {
    // blink 1 by 1 ////////////////////////////////////////
    {
...
void KeedNowChannel::taskSequence5() {
    // blink 2 fill ////////////////////////////////////////
    {
        for (int j = 0; j < cfg.pin_size / 2; ++j) {
...
void KeedNowChannel::taskSequence6() {
...
void KeedNowChannel::sleep(uint32_t _time) {
    if (isr.pressed) return;
    delay(_time);
}

void KeedNowChannel::set(uint8_t _pin, uint8_t _state) {
    if (isr.pressed) return;
...
void KeedNowChannel::setStateHigh(int index, ...) {
    for (int i = 0; i < cfg.pin_size; i++) {
        set(cfg.pin_ptr[i], LOW);
    }
    va_list args;
...
void KeedNowChannel::setStateLow(int index, ...) {
    for (int i = 0; i < cfg.pin_size; i++) {
        set(cfg.pin_ptr[i], LOW);
    }
    va_list args;
...
void KeedNowChannel::off() {
    for (int i = 0; i < cfg.pin_size; i++) {
        set(cfg.pin_ptr[i], LOW);
    }
}

void KeedNowChannel::on() {
    for (int i = 0; i < cfg.pin_size; i++) {
        set(cfg.pin_ptr[i], HIGH);
    }
}
