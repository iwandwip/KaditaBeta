...
#include "KeedTaskSeq.h"

#define setHigh(...) setStateHigh(__VA_ARGS__, -1)
#define setLow(...) setStateLow(__VA_ARGS__, -1)

KeedTaskSeq::KeedTaskSeq()
        : sequence(0), ioTimer(40), taskTemp(nullptr),
          sequences{&KeedTaskSeq::off,
                    &KeedTaskSeq::taskSequence0,
                    &KeedTaskSeq::taskSequence1,
                    &KeedTaskSeq::taskSequence2,
                    &KeedTaskSeq::taskSequence3,
                    &KeedTaskSeq::taskSequence4,
                    &KeedTaskSeq::taskSequence5,
                    &KeedTaskSeq::taskSequence6,
                    &KeedTaskSeq::on} {}

void KeedTaskSeq::init(IOExpander **_ioBase, configuration_t _cfg) {
...
void KeedTaskSeq::run() {
    update();
}

void KeedTaskSeq::setInterruptConfig(interrupt_t _cfg) {
    isr = _cfg;
}

void KeedTaskSeq::changeModes() {
...
void KeedTaskSeq::setBaseDelay(uint32_t _time) {
    ioTimer = _time;
}

void (KeedTaskSeq::*KeedTaskSeq::getSequence(uint8_t index))() {
    return sequences[index];
}

void KeedTaskSeq::taskSequence0() {
    // blink ////////////////////////////////////////
    {
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 15; ++j) {
...
void KeedTaskSeq::taskSequence1() {
    // fill 2 point ////////////////////////////////////////
    {
...
void KeedTaskSeq::taskSequence2() {
    // fill right ////////////////////////////////////////
    {
        for (int i = cfg.pin_size; i > 0; --i) {
            for (int j = 0; j < i; j++) {
...
void KeedTaskSeq::taskSequence3() {
    // fill in ////////////////////////////////////////
    {
        for (int j = 0; j < cfg.pin_size / 2; ++j) {
...
void KeedTaskSeq::taskSequence4() {
    // blink 1 by 1 ////////////////////////////////////////
    {
...
void KeedTaskSeq::taskSequence5() {
    // blink 2 fill ////////////////////////////////////////
    {
        for (int j = 0; j < cfg.pin_size / 2; ++j) {
...
void KeedTaskSeq::taskSequence6() {
...
void KeedTaskSeq::sleep(uint32_t _time) {
    if (isr.pressed) return;
    delay(_time);
}

void KeedTaskSeq::set(uint8_t _pin, uint8_t _state) {
    if (isr.pressed) return;
...
void KeedTaskSeq::setStateHigh(int index, ...) {
    for (int i = 0; i < cfg.pin_size; i++) {
        set(cfg.pin_ptr[i], LOW);
    }
    va_list args;
...
void KeedTaskSeq::setStateLow(int index, ...) {
    for (int i = 0; i < cfg.pin_size; i++) {
        set(cfg.pin_ptr[i], LOW);
    }
    va_list args;
